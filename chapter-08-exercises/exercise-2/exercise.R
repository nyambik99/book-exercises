...
sentence <- tolower("I am in deep deprivation of sunlight and outdoor activities")
...
letter_fix <- str_extract_all(strsplit(sentence, split = ""), "[a-z]+")

# Use the `unique()` function to get a vector of unique letters
unique(letter_fix, fromLast = TRUE)
